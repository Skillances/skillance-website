...
const AdminCategories: React.FC = () => {
  ...
  const columns: Column<CategoryItem>[] = [
    { key: 'name', header: 'Name', render: (c) => (<div className="flex items-center gap-3">{c.color && <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: c.color }} />}<div><p className="text-black font-medium text-sm">{c.name}</p><p className="text-neutral-400 text-xs">{c.slug}</p></div></div>) },
    { key: 'freelancerCount', header: 'Freelancers', render: (c) => <span className="text-neutral-600">{c.freelancerCount ?? '--'}</span> },
    { key: 'isActive', header: 'Status', render: (c) => <StatusBadge status={c.isActive ? 'active' : 'suspended'} label={c.isActive ? 'Active' : 'Inactive'} /> },
    { key: 'displayOrder', header: 'Order', render: (c) => <span className="text-neutral-500">{c.displayOrder}</span> },
    { key: 'actions', header: 'Actions', render: (c) => (<div className="flex items-center gap-2"><Button size="sm" variant="ghost" className="h-7 px-2 text-neutral-400 hover:text-black" onClick={(e) => { e.stopPropagation(); openEdit(c); }}><Edit className="h-3.5 w-3.5" /></Button><Button size="sm" variant="ghost" className="h-7 px-2 text-red-400 hover:text-red-600 hover:bg-red-50" onClick={(e) => { e.stopPropagation(); setDeleteId(c.id); setDeleteOpen(true); }}><Trash2 className="h-3.5 w-3.5" /></Button></div>) },
  ];
    ...
    <div className="space-y-10">
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="bg-white border-neutral-200 text-black sm:max-w-md rounded-2xl shadow-soft-lg">
          <DialogHeader><DialogTitle className="text-black font-serif text-xl">{editId ? 'Edit Category' : 'Create Category'}</DialogTitle></DialogHeader>
          <div className="space-y-5 py-2">
            <div><label className="text-xs text-neutral-400 uppercase tracking-widest font-medium mb-2 block">Name *</label><Input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="bg-white border-neutral-200 text-black rounded-xl focus-visible:ring-neutral-300" /></div>
            <div><label className="text-xs text-neutral-400 uppercase tracking-widest font-medium mb-2 block">Slug</label><Input value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))} placeholder="auto-generated if empty" className="bg-white border-neutral-200 text-black rounded-xl focus-visible:ring-neutral-300" /></div>
            <div><label className="text-xs text-neutral-400 uppercase tracking-widest font-medium mb-2 block">Description</label><Textarea value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} className="bg-white border-neutral-200 text-black min-h-[60px] rounded-xl focus-visible:ring-neutral-300" /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="text-xs text-neutral-400 uppercase tracking-widest font-medium mb-2 block">Color</label><Input type="color" value={form.color || '#171717'} onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))} className="bg-white border-neutral-200 h-10 p-1 rounded-xl" /></div>
              <div><label className="text-xs text-neutral-400 uppercase tracking-widest font-medium mb-2 block">Display Order</label><Input type="number" value={form.displayOrder} onChange={(e) => setForm((f) => ({ ...f, displayOrder: parseInt(e.target.value) || 0 }))} className="bg-white border-neutral-200 text-black rounded-xl focus-visible:ring-neutral-300" /></div>
            </div>
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                <label className="text-xs text-neutral-400 uppercase tracking-widest font-medium">Active</label>
                <button onClick={() => setForm((f) => ({ ...f, isActive: !f.isActive }))} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${form.isActive ? 'bg-black' : 'bg-neutral-200'}`}><span className={`inline-block h-4 w-4 transform rounded-full transition-transform duration-200 ${form.isActive ? 'translate-x-6 bg-white' : 'translate-x-1 bg-neutral-400'}`} /></button>
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs text-neutral-400 uppercase tracking-widest font-medium">Proximity Based</label>
                <button onClick={() => setForm((f) => ({ ...f, isProximityBased: !f.isProximityBased }))} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${form.isProximityBased ? 'bg-black' : 'bg-neutral-200'}`}><span className={`inline-block h-4 w-4 transform rounded-full transition-transform duration-200 ${form.isProximityBased ? 'translate-x-6 bg-white' : 'translate-x-1 bg-neutral-400'}`} /></button>
              </div>
            </div>
          </div>
          <DialogFooter className="gap-2 sm:gap-2 pt-2">
            <Button variant="outline" onClick={() => setFormOpen(false)} className="border-neutral-200 text-neutral-600 hover:bg-neutral-50 hover:text-black rounded-full">Cancel</Button>
